...
class Parser
{
  public function parseDoc(string $docComment, string $namespace = null): array
  {
    ...
    foreach ($matches as $match) {
      $match = $this->parseType($match['type'], $namespace) + $match;
  ...
  public function parseType(string $docType, string $namespace = null): array
  {
    $data = [
      'type' => null,
      'isNullable' => false,
    ...
    if (!$data['isClass'] && isset($namespace) && class_exists($namespace . '\\' . $data['type'])) {
      $data['type'] = $namespace . '\\' . $data['type'];
      $data['isClass'] = true;
    }
